...
[GlobalClass]
public partial class CVarVector3 : CVarBase<Vector3>
{
	public override string DisplayType => "CVar_Vector3";

	[Export]
	protected override Vector3 _defaultValue { get; set; }
	[Export]
	protected override Vector3 _value { get; set; }
	...
	[Export] public Vector3 MinLimitValue { get; private set; } = Vector3.Zero;
	...
	[Export] public Vector3 MaxLimitValue { get; private set; } = Vector3.Zero;
	...
	[Export] Vector3 _minClampValue = Vector3.Zero;

	[ExportSubgroup("Maximum")]
	[Export] bool _clampMaxX = false;
	[Export] bool _clampMaxY = false;
	[Export] bool _clampMaxZ = false;
	[Export] Vector3 _maxClampValue = Vector3.Zero;

	protected override Response<CvarSetResponseStatus, Vector3> ParseValue(ReadOnlySpan<string> args)
	{
		if (!ArgumentParser.TryParseVector3Contextual(args, Value, out Vector3 value, out string error))
		...
		Vector3 requestedPreLimit = value;
